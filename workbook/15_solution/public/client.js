// Data Store
// you could store other data in this object!
const store = {
  toppings: []
};
...
window.addEventListener("DOMContentLoaded", async () => {
  await getToppings().catch(handleError);
  renderToppingsList();
...
async function handleSubmitToppingForm(event) {
  ...
  await createTopping(topping).catch(handleError);
  renderToppingsList();
}

async function handleRemoveClick(event) {
  const toppingName = event.target.parentElement.dataset.topping;
  await deleteTopping(toppingName).catch(handleError);
  renderToppingsList();
}

function handleError(err) {
  console.log(err.message);
  // Could alternatively append an HTML element to let the user know that something is wrong.
}